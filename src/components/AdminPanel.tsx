...
export default function AdminPanel({ teams, teamGolfers, golfers, scores, onDataChange }: Props) {
  ...
  async function handleDeleteTeam(teamId: string) {
    const { error: tgErr } = await supabase.from('team_golfers').delete().eq('team_id', teamId);
    if (tgErr) {
      console.error('Failed to delete team_golfers:', tgErr);
      return;
    }
    const { error: teamErr } = await supabase.from('teams').delete().eq('id', teamId);
    if (teamErr) {
      console.error('Failed to delete team:', teamErr);
      return;
    }
